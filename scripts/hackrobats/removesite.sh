...
mysql -u deploy -e "drop database $machine; drop user $machine@localhost; drop user $machine@dev; drop user $machine@dev.hackrobats.net; flush privileges;"
mysql -u deploy -e "drop database $machine; drop user $machine@localhost; drop user $machine@prod; drop user $machine@orod.hackrobats.net; flush privileges;"
sudo -u deploy ssh deploy@dev "mysql -u deploy -e 'drop database $machine; drop user $machine@localhost; drop user $machine@local; drop user $machine@local.hackrobats.net; flush privileges;'"
...
sudo -u deploy ssh deploy@dev "mysql -u deploy -e 'drop database $machine; drop user $machine@localhost; drop user $machine@prod; drop user $machine@prod.hackrobats.net; flush privileges;'"
sudo -u deploy ssh deploy@prod "mysql -u deploy -e 'drop database $machine; drop user $machine@localhost; drop user $machine@local; drop user $machine@local.hackrobats.net;flush privileges;'"
sudo -u deploy ssh deploy@prod "mysql -u deploy -e 'drop database $machine; drop user $machine@localhost; drop user $machine@dev; drop user $machine@dev.hackrobats.net;flush privileges;'"
