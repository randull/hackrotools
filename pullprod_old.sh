...
sudo -u deploy ssh deploy@prod_old "cd /var/www/$domain/html && sudo rm -rf CHANGELOG.txt COPYRIGHT.txt LICENSE.txt MAINTAINERS.txt UPGRADE.txt"
...
sudo -u deploy ssh deploy@prod_old "cd /var/www/$domain/html && git gc"
...
sudo -u deploy ssh deploy@prod_old "cd /var/www/$domain/html && git commit -a -m \"Preparing Git Repo for Drupal Updates on Local Server\""
sudo -u deploy ssh deploy@prod_old "cd /var/www/$domain/html && git push origin master"
# Git steps on Local
cd /var/www/$domain/html
git status
git diff
